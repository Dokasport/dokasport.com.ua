	...
	public function index() {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			if(isset($uniset['search']['enabled'])) {
				...
				$data = array_merge($data, $this->load->controller('extension/module/uni_new_data', ['type' => 'search', 'filter_data' => $filter_data]));
			} else {
				$data['categories_search'] = $data['manufacturers_search'] = [];
				$data['uni_search'] = false;
			}
			
			$product_total = $this->{isset($uniset['search']['enabled']) ? 'model_extension_module_uni_search' : 'model_catalog_product'}->getTotalProducts($filter_data);

			$results = $this->{isset($uniset['search']['enabled']) ? 'model_extension_module_uni_search' : 'model_catalog_product'}->getProducts($filter_data);
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/search', $url . '&limit=' . (int)$value)
				);
			}
